...
export function WeeklyVehiclesMetrics() {
    ...
    const { data: vehiclesData, isLoading, isError } = useGetWeeklyVehiclesQuery();
    ...
    if (isLoading) return <div>Loading weekly vehicles...</div>;
    if (isError || !vehiclesData) return <div>Error loading weekly vehicles.</div>;
    ...
    return (
        <div style={{
            ...
            <h2 style={{ marginBottom: "1rem" }}>Weekly Vehicles Created</h2>
                    ...
                    <tbody>
                        {vehiclesData.map((week) => (
                            <tr key={week.weekStart} style={{ borderBottom: "1px solid #ddd" }}>
                                ...
                                <td style={{ padding: "8px", color: parrotDarkBlue }}>
                                    {formatNumber(week.vehicleCount)}
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <h3 style={{ marginBottom: "1rem" }}>Vehicles Created Over Time</h3>
